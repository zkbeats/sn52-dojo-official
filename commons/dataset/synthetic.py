...
from commons.obfuscation.obfuscation_utils import obfuscate_html_and_js, JSObfuscator
    ...
    for response in synthetic_qa.responses:
        if hasattr(response.completion, 'files'):
            ...
            if not html_file and not js_file:
                logger.warning(f"No HTML or JS files found for response {response.completion_id}")
        else:
            logger.warning(f"Completion does not have 'files' attribute for response {response.completion_id}")
