...
script="main_validator.py"
proc_name="dojo_subnet_validator_main_process"
args=()
...
then
    echo "jq could not be found. To install see: https://jqlang.github.io/jq/"
    exit 1
fi
...
if [ "$?" -eq 1 ]; then
    while true; do
        ...
        if [ -d "./.git" ]; then
            ...
            latest_version=$(check_variable_value_on_github "tensorplex-labs/dojo-subnet" "template/__init__.py" "__version__ ")
    ...
    echo "Missing package 'jq'. please install for your system first, see: https://jqlang.github.io/jq/"
fi